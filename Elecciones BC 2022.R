...
BC_Diputados_2021[,5:31] <- sapply(BC_Diputados_2021[,5:31], as.numeric)
...
BC_Ayuntamiento_2010[is.na(BC_Ayuntamiento_2010)] <- 0
BC_Ayuntamiento_2013[is.na(BC_Ayuntamiento_2013)] <- 0
BC_Ayuntamiento_2016[is.na(BC_Ayuntamiento_2016)] <- 0
BC_Ayuntamiento_2019[is.na(BC_Ayuntamiento_2019)] <- 0
BC_Ayuntamiento_2021[is.na(BC_Ayuntamiento_2021)] <- 0

BC_Diputados_2010[is.na(BC_Diputados_2010)] <- 0
BC_Diputados_2013[is.na(BC_Diputados_2013)] <- 0
BC_Diputados_2016[is.na(BC_Diputados_2016)] <- 0
BC_Diputados_2019[is.na(BC_Diputados_2019)] <- 0
BC_Diputados_2021[is.na(BC_Diputados_2021)] <- 0

BC_Gubernatura_2013[is.na(BC_Gubernatura_2013)] <- 0
BC_Gubernatura_2019[is.na(BC_Gubernatura_2019)] <- 0
BC_Gubernatura_2021[is.na(BC_Gubernatura_2021)] <- 0
...
aggregate(BC_Diputados_2013$UNIDOS.POR.BAJA.CALIFORNIA, by = list(Category = BC_Diputados_2013$MUNICIPIO), FUN=sum)
aggregate(BC_Gubernatura_2013$UNIDOS.POR.BAJA.CALIFORNIA, by = list(Category = BC_Gubernatura_2013$MUNICIPIO), FUN=sum)

aggregate(BC_Ayuntamiento_2016$PAN, by = list(Category = BC_Ayuntamiento_2016$MUNICIPIO), FUN=sum)
aggregate(BC_Diputados_2016$PAN, by = list(Category = BC_Diputados_2016$MUNICIPIO), FUN=sum)

aggregate(BC_Ayuntamiento_2019$PAN, by = list(Category = BC_Ayuntamiento_2019$MUNICIPIO), FUN=sum)
aggregate(BC_Diputados_2019$PAN, by = list(Category = BC_Diputados_2019$MUNICIPIO), FUN=sum)
aggregate(BC_Gubernatura_2019$PAN, by = list(Category = BC_Gubernatura_2019$MUNICIPIO), FUN=sum)

aggregate(BC_Ayuntamiento_2021$PAN, by = list(Category = BC_Ayuntamiento_2021$MUNICIPIO), FUN=sum)
aggregate(BC_Diputados_2021$PAN, by = list(Category = BC_Diputados_2021$MUNICIPIO), FUN=sum)
aggregate(BC_Gubernatura_2021$PAN, by = list(Category = BC_Gubernatura_2021$MUNICIPIO), FUN=sum)
...
#BC_Diputados_2010 <- subset(BC_Diputados_2010, select=-c(V16))
setnames(BC_Ayuntamiento_2010, old = c("CABC", "CGR", "CRBC"), new = c("Pre - ", "Pre - PRI", "PT"))
setnames(BC_Diputados_2010, old = c("CABC", "CGR", "CRBC"), new = c("Pre - ", "Pre - PRI", "PT"))
...
setnames(BC_Ayuntamiento_2016, old = c("", "PRI", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8","C9", "C10", "C11"), 
         new = c("Pre - PAN", "Pre - PRI", rep(c("Pre - PRI"),each=11)))
setnames(BC_Diputados_2016, old = c("", "PRI", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8","C9", "C10", "C11"), 
         new = c("Pre - PAN", "Pre - PRI", rep(c("Pre - PRI"),each=11)))
